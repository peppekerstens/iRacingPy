...
try:
    df_previous_state = pd.read_csv(driver_indicator_file)
except:
    #scenario first race
...
gold_driver_count = len(df_pec_driver_info_sorted)
...
print(silver_driver_deadzone)
print(gold_driver_deadzone)
if silver_driver_deadzone > 0 and gold_driver_deadzone >0:
    ...
    print('deadzone speed')
    print(silver_deadzone_speed)
    print(gold_deadzone_speed)
    ...
    for index, df_row in df_pec_driver_info_sorted.iterrows():
        ...
        if new_classification == 'Gold':
            driver_indicator = df_indicator['cust_id'] == df_row['cust_id'] 
            df_indicator_driver = df_indicator[driver_indicator]
            percentage = (df_indicator_driver['race_count'] * df_indicator_driver['percentage'] + df_row['percentage']) / df_row['race_count']
        else:
